...
#define DPRAM_PHY_START 	0xED000000
...
static int viper_ota_reset(void);
static struct modemlink_memmap dpram_memmap = {
	...
	.vendor_clear_irq = s5pv210_dpram_int_clear,
	.board_ota_reset = viper_ota_reset,
};
...
static struct modem_data umts_modem_data = {
	...
	.gpio_ap_wakeup = GPIO_DPRAM_INT_N,
...
static struct resource umts_modem_res[] = {
	[0] = {
		...
		.start = IRQ_EINT15,
		.end = IRQ_EINT15,	/* GPIO_PHONE_ACTIVE */
...
static int viper_ota_reset(void)
{
	...
	#ifdef MODEM_IF_DEBUG_STMTS
        printk("[MODEM_IF] %s %d \n",__FUNCTION__,__LINE__);
	printk("[MODEM_IF] %s %d GPIO CP On: %d \n",__FUNCTION__,__LINE__,gpio_cp_on );
	printk("[MODEM_IF] %s %d  GPIO CP rst: %d\n",__FUNCTION__,__LINE__,gpio_cp_rst);
	printk("[MODEM_IF] %s %d  GPIO MIDEM init dpram: %d\n",__FUNCTION__,__LINE__,gpio_ap_wakeup);
	printk("[MODEM_IF] %s %d  GPIO PDA active: %d\n",__FUNCTION__,__LINE__,gpio_pda_active);
	printk("[MODEM_IF] %s %d  GPIO Phone active: %d\n",__FUNCTION__,__LINE__,gpio_phone_active);
	#endif	
